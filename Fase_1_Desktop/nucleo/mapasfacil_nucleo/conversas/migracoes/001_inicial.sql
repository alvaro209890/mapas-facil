-- F1-17 — migração 001: cria o banco de conversas do zero.
--
-- Convenção das migrações (ver `banco.aplicar_migracoes`): cada script abre e
-- fecha a própria transação e grava a própria linha em `schema_versao`. O DDL
-- daqui é idêntico ao de `esquema.sql` — o teste de deriva compara os dois.
